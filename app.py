...
st.set_page_config(
    page_title="Memory-Powered Chat Assistant",
    page_icon="🧠",
    layout="wide",
    ...
    config = {
        ...
        "vector_store": {
            ...
            "config": {
                ...
                "collection_name": "streamlit_memories"
            }
        }    
    }
...
# Chat function with memory and debug option
def chat_with_memories(message, user_id, show_memories=False):
    ...
    # Display retrieved memories if debug is enabled
    if show_memories and memories_str:
        with st.expander("Retrieved Memories"):
            st.markdown(memories_str)
...
with st.sidebar:
    ...
    if not st.session_state.authenticated:
        ...
        if user:
            ...
            # Debug toggle
            st.session_state.show_memories = st.toggle("Show retrieved memories", value=st.session_state.show_memories)
            ...
            model_options = ["gpt-4o-mini", "gpt-4o", "claude-3-sonnet-20240229"]
            selected_model = st.selectbox(
                "Select AI Model",
                options=model_options,
                index=0
            )
            os.environ["MODEL_CHOICE"] = selected_model
...
if st.session_state.authenticated and st.session_state.user:
    ...
    if user_input:
        ...
        # Get AI response
        ai_response = chat_with_memories(
            user_input, 
            user_id, 
    ...
    st.title("Welcome to Memory-Powered Chat Assistant")
    ...
    with col1:
        ...
        st.write("The AI remembers your past conversations and preferences using Qdrant vector database.")
    ...
    st.write("""
    1. **Vector Embeddings**: Your conversations are converted into mathematical representations (vectors) that capture their meaning.
    2. **Semantic Memory**: Qdrant stores these vectors and can find similar conversations based on meaning, not just keywords.
    3. **Contextual Responses**: When you chat, the AI retrieves relevant memories to inform its responses.
    """)
